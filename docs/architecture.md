...
MkDocs `!!!` 提示块作为可编辑结构导入，保留类型、标题和正文缩进；`???`
折叠提示与 `===` 标签页作为完整源码卡片导入。
...
- `export.ts`：局部导出。命中基线且语义未变的块复用 `raw`；原本相邻的块
  复用原间隔和首尾空白，新增或重排块使用文档的默认换行。
...
- `extensions.ts`：在 schema 之上挂载 Vue 节点视图；schema 本身保持无 DOM 依赖，
  供源码保留层与测试使用。富文本图片、媒体和链接按宿主 `context` 解析展示地址，
  Markdown 仍使用节点中的原始相对地址。
